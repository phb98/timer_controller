...
static void node_draw_scr()
{
  ...
  memcpy(&(mainscreen_info.mainscreen.current_time), &current_time, sizeof(ux_rtc_t));
...
static ux_node_t * node_process(ux_event_t evt, const ux_evt_param_t * p_evt_param)
{
  ...
  switch(evt)
  {
    case UX_EVENT_NODE_SWITCH_INTO:
      ...
      ux_time_get_current(&current_time);
      break;
    case UX_EVENT_TIME_UPDATE:
      memcpy(&current_time, &(p_evt_param->time_update.new_time), sizeof(ux_rtc_t));
