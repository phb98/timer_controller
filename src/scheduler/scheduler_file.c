...
static void     scheduler_file_new_file(const char * const file_path);
...
scheduler_ret_t scheduler_file_new_scheduler(const scheduler_t * const p_new_scheduler)
{
  ...
  json_len = p_json_buff - JSON_buffer;
  // Validate if new json is valid
  result = JSON_Validate(JSON_buffer, json_len);
  ...
  if(result == JSONSuccess)
  {
    ...
    scheduler_file_entry_t new_entry = 
    {
      .json_len  = json_len,
      .signature = ENTRY_SIGNATURE,
      .RESERVED  = {0},
    };
    // Write json entry header first
    scheduler_file_append(SCHEDULER_FILE_PATH, (uint8_t *)&new_entry, sizeof(scheduler_file_entry_t));
    if(!scheduler_file_append(SCHEDULER_FILE_PATH, JSON_buffer, json_len))
    {
...
static bool is_entry_equal(const scheduler_t * p_sch_1, const scheduler_t * p_sch_2)
{
  ...
  if(memcmp(&(p_sch_1->trig_time), &(p_sch_2->trig_time), sizeof(rtc_t)) != 0) return false;
